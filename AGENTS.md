...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/yml-sorter`](https://github.com/genzouw/yml-sorter) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の明示的な承認を **MUST** 取得してください。
**承認済みの例外のみ** が第 1 章の自動クローズ規定 (Line 20) の対象外となります。事前承認を得ていない例外提案の PR は、内容の良し悪しにかかわらず引き続き自動的にクローズされます。
